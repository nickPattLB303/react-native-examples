...
2.  **Provider Setup (`App.tsx`):**
    ...
    - `ThemeProvider` from `styled-components/native` is nested _inside_ `PaperProvider`.
    - A theme object (to be defined in `src/theme/theme.ts`) is passed to _both_ providers via their `theme` prop.
