...
> 📲 **(Native Developers):** > **Comparison:** The New Architecture, particularly JSI, moves React Native closer to how native components often interact with the system or each other—more directly and with less marshaling overhead. Fabric's approach to rendering off the main JS thread shares principles with how native UI frameworks manage their rendering pipelines to ensure responsiveness.
> **Key Takeaway:** The New Architecture aims to make React Native feel more "native" in its performance characteristics and interaction model between JS and native code.

> 🌐 **(Web Developers):** > **Comparison:** JSI enabling direct calls between JS and C++ is somewhat analogous to WebAssembly (Wasm) allowing near-native performance for compiled code in the browser, or how Node.js C++ addons can be called from JavaScript. Fabric's concurrency features align with modern React's capabilities for smoother rendering on the web.
> **Key Takeaway:** The New Architecture brings a more sophisticated and performant execution model, addressing limitations that were more apparent when comparing React Native's legacy bridge to web or native execution models.
...
This concludes the content for Module 15. The next step according to the workflow is the Module Challenge, which is already linked in `section-00-introduction.md`:
