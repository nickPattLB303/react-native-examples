...
- **Basic Usage:**
  ...
  // const storage = new MMKV({ id: 'user-preferences', encryptionKey: 'your-secure-key' });
...
- `loadThemePreference`: Fetches the theme from AsyncStorage when the component mounts. `JSON.parse` would be needed if storing complex objects.
- `saveThemePreference`: Saves the selected theme to AsyncStorage using `setItem`.
- The UI allows toggling the theme, and the preference persists across app restarts.
- A button to clear the preference using `removeItem` is also included.

**Example 2: Using `expo-sqlite` for Medication Log**

Imagine SpeedyMeds wants to keep a local log of when users mark their medications as taken.
...
// Opens or creates the database file (e.g., SpeedyMeds.db)
const db = SQLite.openDatabase("SpeedyMeds.db");
...
const MedicationLog: React.FC = () => {
  ...
  const setupDatabase = useCallback(() => {
    db.transaction(
      (tx) => {
        tx.executeSql(
          "CREATE TABLE IF NOT EXISTS medication_log (id INTEGER PRIMARY KEY AUTOINCREMENT, medicationName TEXT, timestamp TEXT);"
        );
      },
      (error) => console.error("DB Setup Error:", error),
  ...
  const loadLogEntries = useCallback(() => {
    db.transaction((tx) => {
      tx.executeSql(
        "SELECT * FROM medication_log ORDER BY timestamp DESC;",
        [],
        (_, { rows: { _array } }) => setLogEntries(_array as LogEntry[]),
        (_, error) => {
  ...
  useEffect(() => {
    setupDatabase();
    loadLogEntries();
  }, [setupDatabase, loadLogEntries]);

  const addLogEntry = () => {
    if (medicationInput.trim() === "") {
      Alert.alert("Input Error", "Medication name cannot be empty.");
      return;
    }
    const currentTime = new Date().toISOString();
    db.transaction((tx) => {
      tx.executeSql(
        "INSERT INTO medication_log (medicationName, timestamp) VALUES (?, ?);",
        [medicationInput, currentTime],
        (_, { insertId }) => {
          console.log("Inserted log entry with ID:", insertId);
          loadLogEntries(); // Refresh the list
          setMedicationInput(""); // Clear input
        },
        (_, error) => {
  ...
  const clearLogs = () => {
    db.transaction((tx) => {
      tx.executeSql(
        ...
        [],
        () => {
          Alert.alert("Success", "All medication logs cleared.");
          loadLogEntries(); // Refresh list
        },
        (_, error) => {
          Alert.alert("Error", "Failed to clear logs.");
          console.error("Error clearing logs:", error);
          return true; // True to rollback transaction on error
        }
      );
    });
  };
  ...
  return (
    <View style={styles.container}>
      ...
      />
      <Button title="Log Medication Taken" onPress={addLogEntry} />
      <FlatList
        data={logEntries}
        keyExtractor={(item) => item.id.toString()}
        renderItem={({ item }) => (
          <View style={styles.logItem}>
            <Text style={styles.logText}>{item.medicationName}</Text>
        ...
        ListEmptyComponent={
          <Text style={styles.emptyText}>No logs yet. Add one!</Text>
        }
      />
      <Button title="Clear All Logs" onPress={clearLogs} color="red" />
    </View>
  );
};
...
const styles = StyleSheet.create({
  ...
  logText: { fontSize: 16 },
  logTimestamp: { fontSize: 12, color: "gray", marginTop: 4 },
...
- `SQLite.openDatabase('SpeedyMeds.db')` opens (or creates if it doesn't exist) a database file.
- `setupDatabase`: In a transaction, it creates the `medication_log` table if it's not already present.
- `loadLogEntries`: Selects all entries from the log table and updates the component's state.
- `addLogEntry`: Inserts a new log entry with the medication name and current timestamp into the database. After successful insertion, it reloads the entries to update the UI.
- `clearLogs`: Deletes all entries from the log table.
- A `FlatList` displays the log entries.

> [!CAUTION]
> Database operations, especially schema changes, should be handled carefully. For production apps, consider using migration strategies to update database schemas as your app evolves, to avoid data loss for existing users.

> 📚 **Official Documentation:**
>
> - [`@react-native-async-storage/async-storage` Documentation](https://react-native-async-storage.github.io/async-storage/)
> - [Expo SQLite (`expo-sqlite`) Documentation](https://docs.expo.dev/versions/latest/sdk/sqlite/)
> - [`react-native-mmkv` GitHub Repository](https://github.com/mrousavy/react-native-mmkv)

### Next Steps

Effective data storage, both online and offline, is crucial for robust applications. In the final section of this module, we will cover testing strategies, helping you ensure the quality and reliability of your React Native code, including features like the ones we've discussed.
